...
class VisionRobotExperiment(VisionExperiment):
    ...
    def get_visualization_system(self, learned_system: System) -> DrakeSystem:
        ...
        e.g. data.  Only one robot has visible geometry, since they are
        redundant.
        ...
        Returns:
            New :py:class:`DrakeSystem` with doubled state, repainted elements,
            and only one robot has visible geometry.
        """
        ...
        if regeneration_is_required or self.visualization_system is None:
            ...
            # Create the learned system with the learned geometry but invisible
            # robot, since the robot is already visible in the base system.
            learned_urdfs = cast(MultibodyLearnableSystem,
                                 learned_system).generate_updated_urdfs('vis')

            robot_no_visual_filepath = file_utils.get_asset(
                INVISIBLE_FRANKA_URDF_ASSET)
            new_filepath = op.join(op.dirname(learned_urdfs['robot']),
                                   INVISIBLE_FRANKA_URDF_ASSET)
            shutil.copyfile(robot_no_visual_filepath, new_filepath)
            learned_urdfs['robot'] = new_filepath

            learned_system = DrakeSystem(
                learned_urdfs, self.get_drake_system().dt)
